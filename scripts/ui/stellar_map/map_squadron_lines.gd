...
static func draw_squadron_lines(canvas: CanvasItem, camera, entities: Dictionary, squadrons: Array, fleet, player_id: String) -> void:
	...
	for sq_raw in squadrons:
		...
		var line_col =Color(0.2, 0.8, 1.0, 0.3)  # Cyan follow line
...
static func _get_ship_screen_pos(fleet_index: int, fleet, entities: Dictionary, camera, player_id: String) -> Vector2:
	if fleet_index < 0:
...
static func _draw_leader_badge(canvas: CanvasItem, pos: Vector2, sq) -> void:
	var font: Font = UITheme.get_font()
	var label ="* %s" % sq.squadron_name
