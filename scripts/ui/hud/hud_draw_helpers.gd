...
static func draw_section_header(ctrl: Control, font: Font, x: float, y: float, w: float, text: String) -> float:
	...
	ctrl.draw_string(font, Vector2(x + 9, y), text, HORIZONTAL_ALIGNMENT_LEFT, -1, 13, UITheme.HEADER)
...
static func get_mode_text(ship: ShipController) -> String:
	if ship == null: return "---"
...
static func get_mode_color(ship: ShipController) -> Color:
