...
# Uses ShaderMaterial (mining_beam.gdshader) with distance attenuation:
# bright/wide at source, fading/narrowing toward target.
# =============================================================================

const BEAM_COLOR := Color(0.3, 1.0, 0.6)
const BEAM_COLOR_HOT := Color(0.6, 1.0, 0.8)
const CORE_RADIUS_SOURCE: float = 0.35   # Source end (bottom) — wide
const CORE_RADIUS_TIP: float = 0.08      # Target end (top) — narrow (~23%)
const GLOW_RADIUS_SOURCE: float = 1.3
const GLOW_RADIUS_TIP: float = 0.25

var _beam_shader: Shader = preload("res://shaders/mining_beam.gdshader")
...
func _build_core_beam() -> void:
	...
	cyl.top_radius = CORE_RADIUS_TIP        # Narrow at target
	cyl.bottom_radius = CORE_RADIUS_SOURCE   # Wide at source
	cyl.height = 1.0
	...
	_core_mat.shader = _beam_shader
	_core_mat.set_shader_parameter("beam_intensity", 3.5)
	_core_mat.set_shader_parameter("source_boost", 2.0)
	_core_mat.set_shader_parameter("attenuation_power", 1.5)
	_core_mat.set_shader_parameter("core_width", 0.3)
	_core_mat.set_shader_parameter("warmup", 0.0)
...
func _build_glow_beam() -> void:
	...
	var cyl := CylinderMesh.new()
	cyl.top_radius = GLOW_RADIUS_TIP         # Narrow at target
	cyl.bottom_radius = GLOW_RADIUS_SOURCE    # Wide at source
	cyl.height = 1.0
	...
	_glow_mat.shader = _beam_shader
	_glow_mat.set_shader_parameter("beam_intensity", 2.0)
	_glow_mat.set_shader_parameter("source_boost", 1.5)
	_glow_mat.set_shader_parameter("attenuation_power", 2.0)  # Faster falloff for glow
	_glow_mat.set_shader_parameter("core_width", 0.15)
	_glow_mat.set_shader_parameter("warmup", 0.0)
...
func _build_impact_effects() -> void:
	...
	_impact_light.light_energy = 1.5
...
func _build_source_effects() -> void:
	...
	_source_light.light_energy = 2.5
...
func _process(delta: float) -> void:
	...
	# Warmup ramp — shader handles pulsing/flicker via TIME
	_warmup = minf(_warmup + delta / 0.4, 1.0)
	var warmup_ease: float = _warmup * _warmup * (3.0 - 2.0 * _warmup)  # smoothstep

	# Push warmup to shader
	_core_mat.set_shader_parameter("warmup", warmup_ease)
	_glow_mat.set_shader_parameter("warmup", warmup_ease)

	# Light flicker
	_impact_light.light_energy = warmup_ease * (1.2 + sin(_pulse_t * 8.0) * 0.5)
	_source_light.light_energy = warmup_ease * (2.2 + sin(_pulse_t * 5.0) * 0.5)
