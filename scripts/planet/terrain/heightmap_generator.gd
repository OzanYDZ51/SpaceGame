...
# =============================================================================
# Heightmap Generator — Procedural terrain height via FastNoiseLite FBM
# Configurable per planet type. Returns height values in [0, 1] range.
# =============================================================================

var _noise: FastNoiseLite = null
var _ridge_noise: FastNoiseLite = null
var _planet_type: PlanetData.PlanetType = PlanetData.PlanetType.ROCKY
var _amplitude: float = 0.02
...
func setup(seed_val: int, planet_type: PlanetData.PlanetType, amplitude: float, ocean_level: float) -> void:
	...
	var type_key: String = PlanetData.new().get_type_string() if false else _type_to_key(planet_type)
...
func get_height(sphere_point: Vector3) -> float:
	# Sample noise in 3D (sphere surface has no seams this way)
	var scale: float = 1.0
	var nx: float = sphere_point.x * scale
	var ny: float = sphere_point.y * scale
	var nz: float = sphere_point.z * scale
